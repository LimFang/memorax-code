...
该命令会沿用当前发布通道并保留配置。交互式更新会探测当前配置中已禁用但可运行的客户端，并
逐个询问是否启用，默认选择“是”；拒绝提示或以非交互方式更新都会保持该客户端为禁用状态。
如果 Codex 是本次更新新启用的客户端，请继续确认 Hook 激活提示。如果新版本修改了插件资产或
skill，请重启或刷新 Codex 和 Claude Code。

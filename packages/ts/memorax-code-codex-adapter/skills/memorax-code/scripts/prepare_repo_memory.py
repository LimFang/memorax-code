...
def is_sidecar_only_memory(memory: Path) -> bool:
    if not memory.exists() or not memory.is_dir() or memory.is_symlink():
        return False
    children = list(memory.iterdir())
    return bool(children) and all(
        (
            child.name == "user-profile"
            and child.is_dir()
            and not child.is_symlink()
        )
        or (
            child.name == "procedure-memory"
            and child.is_dir()
            and not child.is_symlink()
        )
...
def provider_notice(provider: str, cli: str, evidence_state: str, login_hint: str) -> dict[str, Any]:
    ...
    if evidence_state == "auth_required":
        ...
        rerun_line = f"Rerun $memorax-code repo-build to collect {evidence_label} evidence."
        return {
            ...
            "provider_user_notice": (
                ...
                f"run `{login_hint}`, then rerun $memorax-code repo-build."
            ),
    ...
    if evidence_state == "cli_missing":
        ...
        return {
            ...
            "provider_user_notice": (
                f"{body} "
                f"I am continuing with local-only repo memory now. To include {evidence_label} evidence, "
                f"install `{cli}`, run `{login_hint}`, then rerun $memorax-code repo-build."
            ),
            "provider_notice_markdown": notice_markdown(body, login_hint, rerun_line),
            "provider_next_steps": [
                f"Install: {cli}",
...
def main(argv: list[str]) -> int:
    ...
    if memory.exists() and not args.reuse and not is_sidecar_only_memory(memory):
